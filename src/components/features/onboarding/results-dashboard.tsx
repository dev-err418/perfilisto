...
function ResultsSidebar({ tab, onTabChange }: { tab: ResultsTab; onTabChange: (tab: ResultsTab) => void }) {
  ...
  const name = account?.name || t("My account");
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <SidebarMenuButton size="lg" className="data-[state=open]:bg-sidebar-accent">
                ...
                <span className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-semibold">{name}</span><span className="truncate text-xs text-muted-foreground">{account?.email || t("Account")}</span></span>
            ...
            <DropdownMenuContent side={isMobile ? "bottom" : "right"} align="end" sideOffset={4} className="theme-light min-w-56 rounded-lg">
              ...
              <DropdownMenuItem className="focus:bg-neutral-100 focus:text-neutral-900" onSelect={signOut}><LogOut />{t("Sign out")}</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
...
export function ResultsDashboard({ order }: { order: Order }) {
  ...
  const toggleFavorite = async (imageId: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(t("Could not save favorite"));
      const saved = await response.json();
      setFavorites(saved.favorites || []);
    } catch {
      setFavorites(previous);
      toast.error(t("Could not save your favorite. Please try again."));
  ...
  return (
    <SidebarProvider className="theme-light bg-sidebar text-foreground">
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-3 px-4"><SidebarTrigger className="hover:bg-neutral-100 hover:text-neutral-900 dark:hover:bg-neutral-100" /><span className="text-sm font-medium">{tab === "favorites" ? t("Favorites") : t("Results")}</span></header>
        {tab === "favorites" && !photos.length && <div className="grid flex-1 place-content-center gap-3 px-6 py-10 text-center"><Heart className="mx-auto size-9 text-primary" /><h1 className="text-xl font-semibold">{t("Your favorites will appear here")}</h1><p className="text-muted-foreground">{t("Tap the heart on any headshot to save it here.")}</p><button type="button" className={`${ONBOARDING_CONTINUE_BUTTON_CLASS} mx-auto mt-4`} onClick={() => setTab("results")}>{t("Browse results")}</button></div>}
          ...
          {photos.map(photo => {
            ...
            return (
              <div key={photo.id} className="group relative overflow-hidden rounded-2xl">
                <Image src={photo.url} alt={t("Your generated headshot")} width={400} height={600} unoptimized className="aspect-[2/3] w-full rounded-2xl object-cover" />
                <button type="button" aria-label={`${favorite ? t("Remove headshot") : t("Favorite headshot")} ${index}${favorite ? ` ${t("from favorites")}` : ""}`} aria-pressed={favorite} disabled={saving} onClick={() => void toggleFavorite(photo.id)}
                ...
                <div className="absolute inset-x-0 bottom-0 flex justify-center bg-gradient-to-t from-black/50 to-transparent px-3 pb-4 pt-12">
                  <a href={`${photo.url}?download=1`} aria-label={t("Download headshot {v0}", { v0: index })} className="inline-flex h-12 w-full max-w-sm items-center justify-center gap-2 rounded-full bg-white px-3 text-base sm:px-5 font-semibold tracking-[0.2px] text-neutral-900 shadow-lg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"><Download className="size-5 shrink-0" aria-hidden="true" /> {t("Download")}</a>
                </div>
              </div>
            );
          })}
